...
class MaskGIT(Trainer):
    ...
    def __init__(self, args):
        ...
        self.scaler = torch.amp.GradScaler('cuda')                               # Init Scaler for multi GPUs
    ...
    def get_network(self, archi):
        ...
        if archi == "vit":
            ...
            if self.args.vit_folder is not None:
    ...
    def eval(self, **kwargs):
        ...
        # Evaluate the model
        m = self.sae.compute_and_log_metrics(self, **kwargs)
        self.vit.train()
        return m
    ...
    def sample_shortcut(self, init_code=None, nb_sample=50, labels=None, sm_temp=1, w=3,
               randomize="linear", r_temp=4.5, sched_mode="arccos", step=12,
               use_shortcut=False, shortcut_model=None, budget=7):
        """ Generate sample with the MaskGIT model
           :param
            init_code   -> torch.LongTensor: nb_sample x 16 x 16, the starting initialization code
            nb_sample   -> int:              the number of image to generated
        ...
        with torch.no_grad():
            ...
            if isinstance(self.vit, DDP):
                head_weight = self.vit.module.tok_emb.weight.T
                head_bias = self.vit.module.bias
            else:
                head_weight = self.vit.tok_emb.weight.T
                head_bias = self.vit.bias

            # Beginning of sampling, t = number of token to predict a step "indice"
            for indice, t in enumerate(scheduler):
                # start_events[indice].record()
                if mask.sum() < t:  # Cannot predict more token than 16*16 or 32*32
                    t = int(mask.sum().item())
                ...
                with torch.amp.autocast('cuda'):  # half precision
                    if w != 0:
                        # Model Prediction
                        if not use_shortcut or refresh_steps[indice]:
                            logit, feat = self.vit(torch.cat([code.clone(), code.clone()], dim=0),
                                            torch.cat([labels, labels], dim=0),
                                            torch.cat([~drop, drop], dim=0),
                                            ret_last_feat=use_shortcut)
                        else:
                            new_token_ids = new_token_ids.repeat(2, 1)
                            new_token_map_cls = torch.zeros((nb_sample, 1), dtype=torch.bool, device=new_token_map.device)
                            new_token_map = torch.cat([new_token_map, new_token_map_cls], dim=-1)
                            new_token_map = new_token_map.repeat(2, 1)
                            mask_ratio = mask.float().mean().unsqueeze(0).expand(nb_sample*2)
                            feat = shortcut_model(feat, new_token_ids, new_token_map, mask_ratio)
                            logit = torch.matmul(feat, head_weight) + head_bias
                            logit = logit[:, :self.patch_size*self.patch_size, :self.codebook_size]
                        logit_c, logit_u = torch.chunk(logit, 2, dim=0)
                        _w = w * (indice / (len(scheduler)-1))
                        # Classifier Free Guidance
                        logit = (1 + _w) * logit_c - _w * logit_u
                ...
                new_token_ids = code[f_mask].view(nb_sample, -1)
                new_token_map = f_mask.view(nb_sample, -1)
        ...
        # lat_lst = [s.elapsed_time(e) for s, e in zip(start_events, end_events)]
        # for i, lat in enumerate(lat_lst):
        #     print(f"Step {i}: {lat:.1f}ms")

        self.vit.train()
        return x, l_codes, l_mask, latency

    def sample_makedata(self, init_code=None, nb_sample=50, labels=None, sm_temp=1, w=3,
               randomize="linear", r_temp=4.5, sched_mode="arccos", step=12, 
               feat_sink=None, batch_id=0):
        """ Generate sample with the MaskGIT model
           :param
            init_code   -> torch.LongTensor: nb_sample x 16 x 16, the starting initialization code
            nb_sample   -> int:              the number of image to generated
        ...
        with torch.no_grad():
            ...
            generation_order = torch.full((nb_sample, self.patch_size*self.patch_size), -1).to(self.args.device)

            # Beginning of sampling, t = number of token to predict a step "indice"
            cond_all_feat = [[] for _ in range(nb_sample)]
            uncond_all_feat = [[] for _ in range(nb_sample)]
            acc_t = 0
            for indice, t in enumerate(scheduler):
                acc_t += t
                if mask.sum() < t:  # Cannot predict more token than 16*16 or 32*32
                ...
                for i in range(nb_sample):
                    cond_all_feat[i].append(feat[i].cpu())
                    uncond_all_feat[i].append(feat[i + nb_sample].cpu())
                    for decode_index in indice_mask[i]:
                        generation_order[i, decode_index] = indice

            # decode the final prediction
            assert torch.all(code >= 0) and torch.all(code < self.codebook_size), f"code values are out of bounds: min {code.min().item()}, max {code.max().item()}"
            _code = torch.clamp(code, 0,  self.codebook_size-1)
            x = self.ae.decode_code(_code)

            # save features
            flat_code = _code.view(nb_sample, self.patch_size*self.patch_size).cpu()
            uncond_labels = torch.full_like(labels, self.vit.codebook_size + 1 + self.vit.nclass)
            cond_flat_code_with_label = torch.cat([flat_code, labels.unsqueeze(-1).cpu()], dim=1)
            uncond_flat_code_with_label = torch.cat([flat_code, uncond_labels.unsqueeze(-1).cpu()], dim=1)
            label_generation_order = torch.full_like(labels, -1).cpu().unsqueeze(-1)
            generation_order_with_label = torch.cat([generation_order.cpu(), label_generation_order], dim=1)
            for i in range(nb_sample):
                cond_all_feat_i = torch.stack(cond_all_feat[i], dim=0)
                cond_traj = {
                    "__key__": f"{batch_id:05d}_{i:03d}_{labels[i]}_cond",
                    'feat.pth': cond_all_feat_i,  # (15, 1025, 768)
                    "teacher_vq_ids.pth": cond_flat_code_with_label[i],  # (1025,)
                    "generation_order.pth": generation_order_with_label[i]  # (1025,)
                }
                uncond_all_feat_i = torch.stack(uncond_all_feat[i], dim=0)
                feat_sink.write(cond_traj)
                uncond_traj = {
                    "__key__": f"{batch_id:05d}_{i:03d}_{labels[i]}_uncond",
                    'feat.pth': uncond_all_feat_i,
                    "teacher_vq_ids.pth": uncond_flat_code_with_label[i],
                    "generation_order.pth": generation_order_with_label[i]
                }
                feat_sink.write(uncond_traj)
