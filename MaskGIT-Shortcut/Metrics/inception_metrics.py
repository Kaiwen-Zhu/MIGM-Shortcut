...
class MultiInceptionMetrics(Metric):
    ...
    def fid(self, real_features, fake_features):
        ...
        # ).item()
        fid, fid_mu, fid_sigma = self._compute_fid(
            real_features_mean, real_features_cov, fake_features_mean, fake_features_cov
        )
        return fid, fid_mu, fid_sigma
    ...
    def _compute_fid(self, mu1: Tensor, sigma1: Tensor, mu2: Tensor, sigma2: Tensor, eps: float = 1e-6) -> Tensor:
        ...
        # )
        diff_mu = diff.dot(diff).item()
        diff_sigma = torch.trace(sigma1) + torch.trace(sigma2) - 2 * tr_covmean
        diff_sigma = diff_sigma.item()
        return diff_mu + diff_sigma, diff_mu, diff_sigma
    ...
    def compute(self) -> Tensor:
        ...
        if self.compute_unconditional_metrics:
            ...
            # )
            fid, fid_mu, fid_sigma = self.fid(real_features, fake_uncond_features)
            output_metrics["fid_unconditional"] = fid
            output_metrics["fid_unconditional_mu"] = fid_mu
            output_metrics["fid_unconditional_sigma"] = fid_sigma
        ...
        if self.compute_conditional_metrics:
            ...
            fid, fid_mu, fid_sigma = self.fid(real_features, fake_cond_features)
            output_metrics["fid_conditional"] = fid
            output_metrics["fid_conditional_mu"] = fid_mu
            output_metrics["fid_conditional_sigma"] = fid_sigma
