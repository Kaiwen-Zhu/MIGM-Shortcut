...
class Attention(nn.Module):
    ...
    def forward(self, x, past_kv=None, use_cache=False, need_weights=False, key=None, value=None):
        ...
        if head_dim * self.num_heads != embed_dim:
            raise ValueError(f"embed_dim must be divisible by num_heads, got {embed_dim=} and {self.num_heads=}")

        if key is None and value is None:
            qkv = F.linear(x, self.mha.in_proj_weight, self.mha.in_proj_bias)
            q, k, v = qkv.chunk(3, dim=-1)
            q = q.view(bsz, tgt_len, self.num_heads, head_dim).transpose(1, 2)  # (B, H, L, D)
            k = k.view(bsz, tgt_len, self.num_heads, head_dim).transpose(1, 2)
            v = v.view(bsz, tgt_len, self.num_heads, head_dim).transpose(1, 2)
        else:
            ...
            if b is None:
                ...
                b_q, b_k, b_v = b.split(embed_dim, dim=0)

            q = F.linear(x, w_q, b_q)
            k = F.linear(key, w_k, b_k)
            v = F.linear(value, w_v, b_v)
            q = q.view(bsz, tgt_len, self.num_heads, head_dim).transpose(1, 2)  # (B, H, L, D)
            k = k.view(bsz, key.size(1), self.num_heads, head_dim).transpose(1, 2)
            v = v.view(bsz, value.size(1), self.num_heads, head_dim).transpose(1, 2)
        ...
        attn_out = torch.nn.functional.scaled_dot_product_attention(q, k, v, dropout_p=dropout_p, is_causal=False)

        attn_out = attn_out.transpose(1, 2).contiguous().view(bsz, tgt_len, embed_dim)
        attention_value = F.linear(attn_out, self.mha.out_proj.weight, self.mha.out_proj.bias)

        present_kv = (k, v) if use_cache else None
        # return attention_value, attention_weight, present_kv
        return attention_value, attention_weight
...
class MaskTransformer(nn.Module):
    ...
    def forward(self, img_token, y=None, drop_label=None, return_attn=False, ret_last_feat=False):
        """ Forward.
            :param:
        ...
        tok_embeddings = self.tok_emb(input)  # 2b, 1025, 768
        ...
        last_feat = x.clone() if ret_last_feat else None
        ...
        if return_attn:  # return list of attention
            return logit[:, :self.patch_size * self.patch_size, :self.codebook_size], attn, last_feat

        return logit[:, :self.patch_size*self.patch_size, :self.codebook_size], last_feat
